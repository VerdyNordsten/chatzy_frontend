...
export default function Register() {
  ...
  const submitHandler = async (e) => {
    ...
    if (!form.username || !form.email || !form.password) {
      setErrors([{ msg: "All forms must be filled" }])
    } else {
      setErrors([])
      setIsLoading(true)
      ...
      <div className="auth mx-auto">
        ...
        <form onSubmit={submitHandler}>
          <div className="mb-3">
            ...
            <input type="text" className="form-control input-auth" id="username" placeholder="Input your name" onChange={inputChangeHandler} value={form.username} />
          </div>
          <div className="mb-3">
            <label htmlFor="email" className="form-label text-secondary">
              <small>Email</small>
            </label>
            <input type="email" className="form-control input-auth" id="email" placeholder="Input your email" onChange={inputChangeHandler} value={form.email} />
          </div>
          <div className="mb-5">
            <label htmlFor="password" className="form-label text-secondary">
              <small>Password</small>
            </label>
            <input type="password" className="form-control input-auth" id="password" placeholder="Input your password" onChange={inputChangeHandler} value={form.password} />
          </div>
          {isLoading ? (
            <button className="btn bg-blue w-100 text-white p-3 rounded-pill" disabled type="button">
              <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true" /> Loading...
            </button>
          ) : (
            <button type="submit" className="btn bg-blue w-100 text-white p-3 rounded-pill">
              Register
            </button>
          )}
        </form>
        <div className="row title-bottom">
          ...
          <div className="col-4 text-secondary text-center" style={{ marginTop: "-10px" }}>
            Register With
          </div>
          <div className="col-4">
            <div className="line" />
