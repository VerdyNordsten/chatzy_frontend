...
export default function Chat({ activeReceiver, detailReceiver, listChat, onSendMessage, message, setMessage, onDeleteMessage, onEditMessage }) {
      ...
      {activeReceiver ? (
          ...
          <div className="chat-menu-header bg-white py-3 px-5">
            <div className="d-flex">
              {detailReceiver.isLoading ? (
                ...
                <>
                  {detailReceiver.data.photo ? <img className="profile-rounded pointer" src={`${detailReceiver.data.photo}`} alt="Gambar Profile" onClick={toggleDrawer} /> : <img className="profile-rounded pointer" src="https://images227.netlify.app/mernuas/default.jpg" alt="Gambar Profile" onClick={toggleDrawer} />}
                  ...
                  <Drawer open={isOpen} onClose={toggleDrawer} direction="right" className="bla bla bla" size={300}>
                    <div className="p-3">
                      <div className="profile mt-4 profile">
                        <div className="position-relative">{detailReceiver.data.photo ? <img className="profile-rounded" src={`${detailReceiver.data.photo}`} alt="Gambar Profile" /> : <img className="profile-rounded" src="https://images227.netlify.app/mernuas/default.jpg" alt="Gambar Profile" />}</div>
                        <h5 className="fw-bold mt-3">{detailReceiver.data.username}</h5>
                        ...
                        {chat.sender_id === localStorage.getItem("id") ? (
                          <div>
                            <div className="d-flex justify-content-end align-items-end mt-4">
                              ...
                              {chat.photo ? <img className="profile-rounded" src={`${chat.photo}`} alt="Gambar Profile" /> : <img className="profile-rounded" src="https://images227.netlify.app/mernuas/default.jpg" alt="Gambar Profile" />}
                            </div>
                            {!chat.is_deleted && (
                              <div className="d-flex justify-content-end w-100" style={{ marginTop: "-12px" }}>
                                <span
                                  ...
                                  <div className="modal-dialog modal-dialog-centered">
                                    <div className="modal-content">
                                      <div className="modal-header">
                                        <h5 className="modal-title" id="editChatLabel">
                                          Edit Chat
                                        </h5>
                                      ...
                                      <div className="modal-footer">
                                        <button type="button" className="btn btn-secondary" data-bs-dismiss="modal" id="close">
                                          Close
                                        </button>
                                        <button type="button" className="btn bg-blue text-white" onClick={() => onEditMessage(editChat, editChatData)}>
                                          Save changes
                                        </button>
                                      </div>
                                    </div>
          ...
          <div className="chat-menu-form bg-white py-3 px-5">
            <form onSubmit={onSendMessage}>
              <div className="input-group">
                <input className="form-control bg-light border-0" id="message" placeholder="Type your message" value={message} onChange={(e) => setMessage(e.target.value)} />
        ...
        <div className="chat-menu col-8 col-md-9 d-flex flex-column">
          <div className="chat-menu-blank p-4">
            <h5 className="text-secondary">Please select a chat to start messaging</h5>
          </div>
        </div>
      )}
    </>
